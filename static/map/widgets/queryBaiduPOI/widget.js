//模块：
var widget_queryBaiduPOI = L.widget.bindClass(
  L.widget.BaseWidget.extend({
    ...
    queryPOI: function() {
      ...
      $.ajax({
        ...
        success: function(data) {
          ...
          var pois = data.results;
          if (pois && pois.length > 0 && pois[0].num > 0) that.workShowCitys.showResult(pois);
          else that.showPOIPage(pois, data.total);
        },
        error: function(data) {
          toastr.error('请求出错(' + data.status + ')：' + data.statusText);
        }
    ...
    workShowCitys: {
      ...
      showPOIPage: function() {
        ...
        for (var index = startIdx; index < endIdx; index++) {
          ...
          inhtml += '<div class="querybar-site" onclick="widget_queryBaiduPOI.queryPOIForCity(\'' + _mc + '\')"> <div class="querybar-sitejj"> <div style=" float: left;">' + _mc + '</div> <div style=" float: right;  ">约' + item.num + '个结果</div></div> </div>';
        }

        //分页信息
        inhtml += '<div class="querybar-page"><div class="querybar-fl">找到<strong>' + this.counts + '</strong>个结果</div><div class="querybar-ye querybar-fr">' + this.thispage + '/' + this.allpage + '页  <a href="javascript:widget_queryBaiduPOI.workShowCitys.showFirstPage()">首页</a> <a href="javascript:widget_queryBaiduPOI.workShowCitys.showPretPage()">&lt;</a>  <a href="javascript:widget_queryBaiduPOI.workShowCitys.showNextPage()">&gt;</a> </div></div>';
    ...
    showPOIPage: function(data, counts) {
      ...
      if (this.counts == 0) {
        ...
        for (var index = 0; index < this.arrdata.length; index++) {
          ...
          inhtml += '<div class="querybar-site" onclick="widget_queryBaiduPOI.showDetail(\'' + _id + '\')"> <div class="querybar-sitejj"> <h3>' + item.index + '、' + _mc + '</h3> <p>' + (item.address || '') + '</p> </div> </div>';
        ...
        //分页信息
        var _fyhtml;
        if (this.allpage > 1) _fyhtml = '<div class="querybar-ye querybar-fr">' + this.thispage + '/' + this.allpage + '页  <a href="javascript:widget_queryBaiduPOI.showFirstPage()">首页</a> <a href="javascript:widget_queryBaiduPOI.showPretPage()">&lt;</a>  <a href="javascript:widget_queryBaiduPOI.showNextPage()">&gt;</a> </div>';
        else _fyhtml = '';
    ...
    showDetail: function(id) {
      ...
      if (item.telephone) inHtml += '<p>电话：' + item.telephone + '</p>';
      if (item.address) inHtml += '<p>地址：' + item.address + '</p>';
      if (item.detail_info) {
        if (item.detail_info.tag) inHtml += '<p>类别：' + item.detail_info.tag + '</p>';
      ...
      if (item.layer) this.centerAt(item.layer);
      else toastr.warning('"' + name + '"无地理坐标信息！');
    },
    layerWork: null,
    ...
    showPOIArr: function(arr) {
      ...
      if (layer.getLayers().length > 0) this.map.fitBounds(layer.getBounds());
    },
