...
<script>
  ...
  var trajectoryObj, trajectoryLine;
  export default {
    data() {
      return {
        currentPage1: 1,
        ...
        trajectoryMsg: {
          'btnShow': [
            { 'title': '查询', 'fn': 'isGoPlayTrajectory' }
          ],
          'options': [
            {
              'status': 5,
    ...
    methods: {
      ...
      //是否播放轨迹
      isGoPlayTrajectory(itemOps) {
        if(!itemOps.sip) {
          this.$message.error('请选择人/车');
          return false;
        }
        if(itemOps.startTime && !itemOps.endTime) {
        ...
        if(this.arrTrajectory != 0) {
          this.$confirm('有轨迹正在播放，需要停止播放当前用户/车辆轨迹吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
          }).then(() => {
            this.mapGlobal.removeLayer(trajectoryObj);
            this.mapGlobal.removeLayer(trajectoryLine);
            this.arrTrajectory = [];
            this.trajectoryBackFn(itemOps);
          }).catch(() => {
            this.isShowTrajectory = false;
          });
        } else {
        ...
        this._getInfo({
          ...
          callback: res => {
            ...
            res.forEach((item, index) => {
              ...
              this.arrTrajectory.push(subArr);
            })
            this.speedArr = [];
            if(itemOps.speed == "low") {
              for(var i = 0; i < this.arrTrajectory.length; i++) {
            ...
            this.isShowTrajectory = false;
            if(this.arrTrajectory.length != 0) {
              this.mapGlobal.setView(this.arrTrajectory[0], 16);
              ...
              trajectoryObj = this.util.movingMarker(this.arrTrajectory, this.speedArr, { autostart: true, icon: myIcon }).addTo(this.mapGlobal);
              trajectoryLine = L.polyline(this.arrTrajectory, { color: 'red' }).addTo(this.mapGlobal);
              trajectoryObj.on('end', () => {
                console.log('轨迹回放完成');
                this.mapGlobal.removeLayer(trajectoryObj);
                this.mapGlobal.removeLayer(trajectoryLine);
                this.arrTrajectory = [];
